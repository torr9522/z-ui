...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/torr9522/z-ui/z-ui/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    LOGE "BBR helper is not bundled in this repository. Please install a reviewed BBR script manually if needed."
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/torr9522/z-ui/z-ui/x-ui_en.sh
    if [[ $? != 0 ]]; then
        echo ""
